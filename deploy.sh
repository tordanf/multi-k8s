...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=tordanf/multi-server:$SHA
kubectl set image deployments/client-deployment client=tordanf/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=tordanf/multi-worker:$SHA
